...
type AuthContextValue = {
  ...
  // refreshToken храним отдельно, чтобы обновлять access без повторного логина
  refreshToken: string | null;
  username: string | null;                          // username — логин тренера
  role: "TRAINER" | "ATHLETE" | null;
  userId: string | null;
  // login принимает пару access/refresh и кладёт её в state + localStorage
  login: (accessToken: string, refreshToken: string, username: string, role: "TRAINER" | "ATHLETE", userId: string) => void;
...
// refresh token используется для silent-refresh access token
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // refresh token живёт дольше access, поэтому восстанавливаем его при старте приложения
  ...
  const login = (newAccessToken: string, newRefreshToken: string, user: string, newRole: "TRAINER" | "ATHLETE", newUserId: string) => {
